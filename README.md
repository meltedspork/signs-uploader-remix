...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-app-w-db-7a8c
  fly apps create remix-app-w-db-7a8c-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-app-w-db-7a8c
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-app-w-db-7a8c-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-app-w-db-7a8c-staging' does not match app name in config file 'remix-app-w-db-7a8c'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-app-w-db-7a8c-db
  fly postgres attach --app remix-app-w-db-7a8c remix-app-w-db-7a8c-db

  fly postgres create --name remix-app-w-db-7a8c-staging-db
  fly postgres attach --app remix-app-w-db-7a8c-staging remix-app-w-db-7a8c-staging-db
  ```
